...
namespace Sample
{
    public static class LessonTypes
    {
         ...
         * 1 радиан равен 57 градусов 17 минут 45 секунд = 206_265 секунд
         *
         */
        public static double AngleToRadian(int deg, int min, int sec) => (double)((deg * 3600) + (min * 60) + sec) / 206_265;
        ...
        {
            char[] ar = number.ToString().ToCharArray();
            return int.Parse(ar[ar.Length - 3].ToString());
        }
         ...
         */
        //а мне тупо лень разбираться с мантисой и почему 
        // Expected: 110.0d +/- 0.01d
        //But was:  110.47130674412968d
        public static double CalcMoney(int initialMoney, int percent, int years) =>
            initialMoney * Math.Pow(1 + ((double)percent / 100) / 12, years * 12);
        ...
        public static int RevertNumber(int number)
        {
            char[] ar = number.ToString().ToCharArray();
            Array.Reverse(ar);
            return Convert.ToInt32(new String(ar));
        }
    }
}